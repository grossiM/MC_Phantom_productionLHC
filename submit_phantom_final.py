...
def modifySubmitfileIntelCompiler (submitfilename, pdfgridfolder, phantompdflib):
    ...
    submitfile.write ('export LHAPDF=/afs/cern.ch/work/m/mgrossi/local/share/LHAPDF\n')
...
#    submitfile.write ('export PDFLIBDIR=' + pdfgridfolder + '/../../lib\n')
    submitfile.write ('export PDFLIBDIR=/afs/cern.ch/work/m/mgrossi/local/lib\n')
...
# pdfgridfolder = os.environ['LHAPDF_DATA_PATH']
    pdfgridfolder = '/afs/cern.ch/work/m/mgrossi/local/share/LHAPDF'
    ...
    execute ('cp ' + workingfolder + '/' + phantomfolder + '/tools/setupdirall_6_nob.pl ' + workingfolder, debugging)

    channel = config.get ('generation','channel')
    command = './setupdirall_6_nob.pl'
...
##########################################################################################################from here till produce events
    ...
    os.chdir (rootfolder)
    sys.exit (0) #check questa chiusura
...
#    runFile.write('#!/bin/bash\n')
    ...
    modifySubmitfileIntelCompiler ('gen1/run', "", "")
    #modifySubmitfileIntelCompiler (submitfilename, pdfgridfolder, phantompdflib)

    # Now run the phantom tool to generate the folders
    #"./gendir.scr -l LOCATION -q QUEUE -d NDIR -i INPUTDIR"
    #QUEUE = local queue (TO: slow; CERN: espresso, microcentury, longlunch, workday, tomorrow, testmatch, nextweek)"
    #execute ("./gendir.scr -l CERN -q " + config.get ('submission', 'queue') +  " -d " + str(ngenerations) + " -i `pwd`", debugging)
    execute ("./gendir.scr -l CERN -q testmatch -d " + str(ngenerations) + " -i `pwd`", debugging) #hardcode testmatch as the time needed
